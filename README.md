...
当前处于 **来源/分类/标签筛选与排序能力已交付，下一步继续推进摘要/片段展示，并补强导入校验**。

- 前台 `/search` 已支持来源、分类、标签三类筛选与排序方式选择：搜索结果页会渲染对应下拉框、保留已选值，并在筛选生效时展示当前筛选摘要
- 首页统一搜索入口现已同步支持来源、分类、标签三类筛选与排序方式选择，下拉选项来自当前启用且存在公开条目的来源 / 分类 / 标签
- `/search` 对非法 `source`、`category`、`tag`、`sort` 参数已显式安全降级：未知值会退回默认语义，不会错误收窄结果或进入异常排序状态
